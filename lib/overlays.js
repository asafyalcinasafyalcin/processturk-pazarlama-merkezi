...
// v2.1 — 9:16 overlay/kart üreticisi. Video marka renkleri (Asaf): navy #1A2B5F, red #D71920.
// Tüm metin SVG→PNG (sharp). Logo TÜM sahnelerde sağ-üst sabit (gerçek ProcessTürk PNG'si).
export const W = 1080, H = 1920;
const NAVY = '#1A2B5F', NAVY2 = '#22356f', RED = '#D71920';
...
export async function ctaCard({ title, whatsapp, web, logoPath, rtl = false }) {
  ...
  const svg = `<svg width="${W}" height="${H}" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="${W / 2}" y="${btnY + 230}" font-family='${FONT}' font-size='44' font-weight='600' fill='#ADC9EB' text-anchor='middle'>${esc(web || 'processturk.com')}</text>
  </svg>`;
  return await withCornerLogo(await svgToPng(svg), logoPath);
}
