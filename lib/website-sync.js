...
export async function syncWebsiteProducts({ force = false, maxImageDownloads = 6 } = {}) {
  ...
  for (const item of feed.products) {
    ...
    const gallery = [];
    for (const g of (item.gallery || []).slice(0, 4)) {
  ...
  fs.writeFileSync(productsJsonPath(), JSON.stringify(records, null, 2) + '\n', 'utf-8');
