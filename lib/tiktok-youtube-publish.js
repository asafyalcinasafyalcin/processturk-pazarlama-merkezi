 ...
 */
/**
 * `YOUTUBE_KANAL_KANITI` = `<refresh token sha256 ilk 12>:<channelId>:<YYYY-AA-GG>`
 * Kanıt YALNIZ şu üç şart birden sağlanırsa geçerlidir:
 *   ① iz, ŞU ANKİ refresh token'ın izine eşit  ② kanal, beklenen kanala eşit
 *   ③ biçim doğru.  Aksi hâlde null → kapı fail-closed kalır.
 */
function kayitliKanit(beklenenKanal) {
  const ham = String(process.env.YOUTUBE_KANAL_KANITI || '').trim();
  if (!ham) return null;
  const [iz, kanal, tarih] = ham.split(':');
  if (!iz || !kanal || !tarih) return null;
  const rt = process.env.GOOGLE_REFRESH_TOKEN || '';
  if (!rt) return null;
  const simdikiIz = createHash('sha256').update(rt).digest('hex').slice(0, 12);
  if (iz !== simdikiIz) {
    console.warn('⚠️ YOUTUBE_KANAL_KANITI BAŞKA bir jetona ait (iz uyuşmuyor) — yok sayıldı.');
    return null;
  }
  if (kanal !== beklenenKanal) {
    console.warn('⚠️ YOUTUBE_KANAL_KANITI beklenen kanaldan FARKLI kanal diyor — yok sayıldı.');
    return null;
  }
  return { kanal, tarih };
}
...
async function kanalDogrulaOnce(accessToken, beklenenKanal) {
  ...
  if (!r.ok) {
    ...
    if (sebep === 'insufficientPermissions' || r.status === 401) {
      // ── KAYITLI KANIT YOLU (2026-08-29) ──────────────────────────────────
      // `youtube.upload` kapsamı `channels.list` çağıramaz. Ama kanal, jetonun
      // KENDİ yükleme cevabından da ölçülebilir: `videos.insert` yanıtı
      // `snippet.channelId` döner. Bu bir tahmin DEĞİL, gerçek bir ölçümdür —
      // yalnız canlı değil, KAYITLI olanıdır.
      //
      // Kanıt jetona BAĞLIDIR (`<refresh token sha256 ilk 12>:<kanal>:<tarih>`).
      // Jeton değişirse iz değişir, kanıt kendiliğinden GEÇERSİZ olur ve kapı
      // yine fail-closed'a döner. Yani "bir kere doğruladık, hep açık" DEĞİLDİR.
      //
      // ⚠️ Bu bir ödündür: canlı ölçüm her zaman üstündür. `youtube.readonly`
      //    kapsamlı bir jeton geldiği anda bu dal hiç çalışmaz.
      const kanit = kayitliKanit(beklenenKanal);
      if (kanit) {
        console.warn(`⚠️ YouTube kanalı CANLI ölçülemedi (dar kapsam) — ${kanit.tarih} `
          + `tarihli KAYITLI kanıtla devam ediliyor (${kanit.kanal}).`);
        return kanit.kanal;
      }
      throw new Error(
        'YouTube kanalı DOĞRULANAMADI — jeton yalnız `youtube.upload` kapsamıyla alınmış, '
        + '`channels.list` çağıramıyor ve bu jeton için KAYITLI kanıt da yok. '
        + 'Doğrulanamayan kanala yükleme YAPILMAZ. Çözüm: `youtube.readonly` kapsamı ekle '
        + 'ya da kanal probu koşup YOUTUBE_KANAL_KANITI değerini yaz.');
    }
