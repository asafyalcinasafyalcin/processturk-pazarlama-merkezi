...
function mesajlariNormalize(mesajlar, sistem) {
  ...
  for (const m of (mesajlar || [])) {
    ...
    if (icerik == null) throw new LLMHatasi("Mesaj içeriği boş olamaz");
    out.push({ role: rol, content: icerik });
  }
...
// ── Ortak HTTP ───────────────────────────────────────────────────────────────
async function istek(url, { basliklar, govde, zamanAsimi, saglayici, denemeler }) {
  let sonHata;
 ...
 * @returns {Promise<{metin:string, json:any, model:string, saglayici:string, kullanim:object}>}
 */
export async function sohbet({
  mesajlar, sistem, model, modelEnv, json, sicaklik, maxToken,
  saglayici, zamanAsimi = 60000, denemeler = 2, env: envOzel,
} = {}) {
  ...
  if (s.bicim === "anthropic") {
    ...
    const y = await istek(`${s.taban}/messages`, {
      ...
      govde, zamanAsimi, saglayici: s.ad, denemeler,
    });
  ...
  if (s.bicim === "fal") {
    ...
    const y = await istek(s.taban, {
      ...
      zamanAsimi, saglayici: s.ad, denemeler,
    });
  ...
  if (json) {
    ...
    if (govde.response_format.type === "json_object"
      && !msj.some((m) => /json/i.test(String(m.content)))) {
  ...
  const y = await istek(`${s.taban}/chat/completions`, {
    basliklar, govde, zamanAsimi, saglayici: s.ad, denemeler,
  });
  const metin = y.choices?.[0]?.message?.content ?? "";
  return paketle(metin, json, secilenModel, s.ad, {
 ...
 */
export async function gomme({ girdiler, model, zamanAsimi = 60000, env: envOzel } = {}) {
  const env = envOzel || ortam();
  ...
  const y = await istek("https://api.openai.com/v1/embeddings", {
    ...
    zamanAsimi, saglayici: "openai", denemeler: 2,
  });
  return {
