...
// Ek durum: manual_action_required → assisted (elle) paket üretildi, gerçek yayın YOK;
//           kullanıcı elle yükleyip sonra 'published' olarak işaretleyebilir.
// Ek durum: cancelled → beğenilmedi, kuyruktan ÇIKARILDI ama SİLİNMEDİ (geri alınabilir).
//           Yayınlanmaz; "beğendiğini sürdür, beğenmediğini iptal et" akışının duraklama hâli.
// Şekil: { items: [ {id, slug, platform, lang, variantId, caption, videoUrl,
//                    scheduledAt, status, publishedAt, result, publishAs} ] }
//
// publishAs: LinkedIn'de hangi kimlikle yayınlanacağı — 'organization' (ProcessTürk
// şirket sayfası) | 'person' (Asaf kişisel profili). Yalnız LinkedIn için anlamlı.
// Belirtilmezse Sosyal Yayın Servisi'ndeki LINKEDIN_PUBLISH_AS varsayılanı geçerlidir.
...
export async function addItem(item) {
  ...
  const rec = {
    ...
    // LinkedIn kimliği: 'organization' | 'person' | null (servis varsayılanı)
    publishAs: item.publishAs === 'organization' || item.publishAs === 'person' ? item.publishAs : null,
