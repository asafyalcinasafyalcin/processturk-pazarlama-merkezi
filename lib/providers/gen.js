...
export async function genImage(o) {
  ...
  await setCached('image', cacheOpts, result).catch(() => {});
  return result;
}
...
export async function genVoice(o) {
  const cacheOpts = { text: o.text, voiceId: o.voiceId || o.voice };
  if (!o.force) {
    const hit = await getCached('voice', cacheOpts);
    if (hit) return { ...hit, fromCache: true };
  }
  const result = await withFallback('VOICE', () => hf.generateVoice(o), () => fal.generateVoice(o));
