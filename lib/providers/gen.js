...
// Görsel motoru: çağrı-başına o.provider > env GEN_IMAGE_PROVIDER/GEN_PROVIDER.
// 'openai' | 'higgsfield' | 'fal' döner.
function imageProviderFor(o) {
  if (o && typeof o.provider === 'string') return o.provider.toLowerCase();
  return genProvider('IMAGE');
}

export async function genImage(o) {
  // OpenAI gpt-image-1 birincilse: image-edit yerel dosya yolu ister → reference_image'ı
  // fal URL'ine ÇEVİRME. Başarısızsa aşağıdaki HF/fal yedek zincirine düşer.
  if (imageProviderFor(o) === 'openai') {
    try {
      return await openai.generateImage(o);
    } catch (oaiErr) {
      console.warn(`[gen] OpenAI gpt-image-1 başarısız (${oaiErr.message.slice(0, 100)}) → HF/fal yedeğine düşülüyor`);
      // fallthrough: yedek zincir yerel referansı kendisi fal URL'ine çevirir (aşağıda).
    }
  }
