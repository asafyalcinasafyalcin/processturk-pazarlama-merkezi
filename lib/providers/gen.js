...
async function withFallback(cap, hfFn, falFn) {
  ...
  if (!hfCredentialsOk()) {
    console.warn(`[gen] Higgsfield credentials yok → fal yedeği deneniyor (${cap})`);
    return falFn();
  }
  try {
    return await hfFn();
  } catch (hfErr) {
    console.warn(`[gen] Higgsfield ${cap} başarısız (${hfErr.message.slice(0, 100)}) → fal yedeği deneniyor`);
    try {
      return await falFn();
    } catch (falErr) {
...
export async function genImage(o) {
  ...
  if (!isHF('IMAGE') && o.reference_image && !isHttp(o.reference_image)) {
