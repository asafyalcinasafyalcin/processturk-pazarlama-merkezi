...
function ensureConfigured() {
  ...
  if (!key) {
    throw new Error('FAL_KEY tanımlı değil. .env.local dosyasına ekleyin (tek .env.local).');
  }
...
// Güvenli fallback modeli: bilinmeyen id gelince en UCUZ'a (flux-schnell / seedance-fast)
// SESSİZCE düşmek Asaf'ın kalite şikayetinin kaynağıydı. Artık bilinmeyen id GÖRÜNÜR
// uyarı verir ve dengeli-güvenli modele (flux-dev / seedance-2) çekilir — en ucuza kaçmaz.
const FAL_IMAGE_FALLBACK = FAL_IMAGE_MODELS.find((m) => m.id === 'flux-dev') || FAL_IMAGE_MODELS[0];
const FAL_VIDEO_FALLBACK = FAL_VIDEO_MODELS.find((m) => m.id === 'seedance-2') || FAL_VIDEO_MODELS[0];

export function findImageModel(id) {
  const m = FAL_IMAGE_MODELS.find((x) => x.id === id);
  if (m) return m;
  if (id) console.warn(`[fal] Bilinmeyen görsel model "${id}" → güvenli fallback "${FAL_IMAGE_FALLBACK.id}" (en ucuza DÜŞÜLMEDİ)`);
  return FAL_IMAGE_FALLBACK;
}
export function findVideoModel(id) {
  const m = FAL_VIDEO_MODELS.find((x) => x.id === id);
  if (m) return m;
  if (id) console.warn(`[fal] Bilinmeyen video model "${id}" → güvenli fallback "${FAL_VIDEO_FALLBACK.id}" (en ucuza DÜŞÜLMEDİ)`);
  return FAL_VIDEO_FALLBACK;
}
...
export async function generateVideo(params = {}) {
  ...
  if (model.endpoint.startsWith('bytedance/seedance-2.0')) {
    // Çözünürlük tier'dan gelir (video/route → videoResolutionForTier). Parametre yoksa
    // 720p'ye düşer (yalnız doğrudan/eski çağrılar için geriye-uyum). Premium yolu 1080p geçer.
