...
const HF_IMAGE_MODELS = ['gpt_image_2', 'flux_2', 'flux_kontext', 'grok_image', 'text2image_soul_v2', 'image_auto', 'cinematic_studio_image'];
...
export async function generateImage({ prompt, aspect_ratio = '9:16', model = 'gpt_image_2' }) {
  const hfModel = HF_IMAGE_MODELS.includes(model) ? model : 'gpt_image_2';
  const ratio = hfRatio(aspect_ratio);
  const out = await runHF(['generate', 'create', hfModel, '--prompt', prompt, '--aspect_ratio', ratio, '--wait', '--wait-timeout', '10m']);
