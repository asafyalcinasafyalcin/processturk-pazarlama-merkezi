...
export function runHF(args, { timeoutMs = 1200000 } = {}) {
  return new Promise((resolve, reject) => {
    ...
    p.on('close', (code) => {
      if (code !== 0) {
        const raw = (err || out).slice(-400);
        // Oturum/yetki hatasını anlamlı mesaja çevir (ham "Forbidden"/"session expired" yerine)
        if (/session expired|auth login|unauthorized|\b401\b|not authenticated|please log ?in/i.test(raw)) {
          return reject(new Error('Higgsfield oturumu süresi dolmuş — yeniden giriş gerekli (higgsfield auth login). Kredi/oturum kontrolü için Ayarlar.'));
        }
        if (/insufficient|no credits|not enough credit|quota/i.test(raw)) {
          return reject(new Error('Higgsfield kredisi yetersiz — hesaba kredi yükleyin (higgsfield.ai).'));
        }
        return reject(new Error(`higgsfield ${code}: ${raw}`));
      }
      resolve(out);
    });
  });
...
export function findUrl(node, exts) {
...
export function parseResult(stdout, exts) {
  let data = null;
...
export const IMG = ['.png', '.jpg', '.jpeg', '.webp'];
export const VID = ['.mp4', '.mov', '.webm'];
export const AUD = ['.mp3', '.wav', '.m4a', '.aac', '.ogg'];
...
// resolution tier'dan gelir (video/route → videoResolutionForTier). Varsayılan 720p yalnız
// doğrudan/eski çağrılar içindir (geriye-uyum); premium yolu 1080p geçer.
...
// Ses stili başına ElevenLabs parametreleri (Higgsfield CLI turbo-v2.5)
const VOICE_STYLE_PARAMS = {
  satis:  { stability: '0.3', style_exaggeration: '0.8' },
  bilgi:  { stability: '0.6', style_exaggeration: '0.3' },
  maskot: { stability: '0.2', style_exaggeration: '1.0' },
};

// Dil → ElevenLabs language_code (aksan düzeltme)
const LANG_CODE_MAP = {
  tr: 'tr', en: 'en', ar: 'ar', fr: 'fr', ru: 'ru',
};

export async function generateVoice({ text, voice, voiceId, lang, voiceStyle }) {
  ...
  const args = ['generate', 'create', 'text2speech_v2', '--model', TTS_MODEL,
    '--voice_id', id, '--voice_type', 'preset', '--prompt', text, '--wait', '--wait-timeout', '8m'];

  // Dil kodu — aksansız konuşma (TR için TR sesi, AR için AR sesi vb.)
  const langCode = LANG_CODE_MAP[lang];
  if (langCode) args.push('--language_code', langCode);

  // Ses stili parametreleri
  const styleParams = VOICE_STYLE_PARAMS[voiceStyle] || VOICE_STYLE_PARAMS.satis;
  args.push('--stability', styleParams.stability, '--style_exaggeration', styleParams.style_exaggeration);

  const out = await runHF(args);
  const r = parseResult(out, AUD);
  return { provider: 'higgsfield', model: 'text2speech_v2', voice: id, lang, voiceStyle, url: r.url };
}
