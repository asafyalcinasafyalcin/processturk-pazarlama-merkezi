...
export async function generateImage({ prompt, aspect_ratio = '9:16', model = 'gpt_image_2' }) {
  const out = await runHF(['generate', 'create', model, '--prompt', prompt, '--aspect_ratio', aspect_ratio, '--wait', '--wait-timeout', '10m']);
  const r = parseResult(out, IMG);
  return { provider: 'higgsfield', model, url: r.url };
}

// image-to-video: imagePath (yerel, otomatik yüklenir) veya image_url
export async function generateVideo({ prompt, imagePath, image_url, aspect_ratio = '9:16', duration = 10, resolution = '720p', generate_audio = false }) {
  ...
  const ref = imagePath || image_url;
  if (ref) args.push('--image', ref);
  const out = await runHF(args, { timeoutMs: 1500000 });
  const r = parseResult(out, VID);
  return { provider: 'higgsfield', model: 'seedance_2_0', url: r.url };
}

export async function generateVoice({ text, voice, voiceId }) {
  const id = voiceId || voice || process.env.HF_VOICE_ID;
  ...
  return { provider: 'higgsfield', model: 'text2speech_v2', voice: id, url: r.url };
}
